...
internal sealed record CameraSourceData(CameraSourceEvent[] Events);
...
internal static class CameraSourceReader
{
    ...
    internal static CameraSourceData Load(string path, CancellationToken cancellationToken = default)
    {
    ...
    private sealed class Parser
    {
        ...
        internal CameraSourceData Complete() => new(_events.ToArray());
        ...
        private void AcceptActions(ref Utf8JsonReader reader)
        {
            ...
            if (reader.TokenType == JsonTokenType.StartObject)
            {
                _action = new ActionBuilder
                {
                    ...
                    Duration = 1.0,
                    RelativeTo = "Player",
                    Position = new CameraSourcePosition(0.0, 0.0),
                    Rotation = 0.0,
                    Zoom = 100.0,
                    Ease = "Linear"
                };
        ...
        private void AcceptPosition(ref Utf8JsonReader reader)
        {
            if (reader.TokenType == JsonTokenType.EndArray)
            {
                _action.Position = new CameraSourcePosition(_positionX, _positionY);
                _mode = ParserMode.Action;
                return;
            }
        ...
        {
            if (!_action.HasFloor || !string.Equals(_action.EventType, "MoveCamera", StringComparison.Ordinal))
                return;
        ...
        private enum ParserMode { Root, Actions, Action, Position, Skip }
...
{
    private readonly record struct PendingEvent(CameraSourceEvent Source, double StartTime, double DurationSeconds);
    private readonly record struct AxisTarget(bool PlayerRelative, float Value);
    private readonly record struct CameraState(float X, float Y, float Rotation, float Zoom);
    ...
    internal static NativeCameraEvent[] Build(LevelDocument level, TimingMap timingMap)
    {
        ...
        List<CameraSourceEvent> live = BuildLiveEvents(level, CameraMetadataCache.Get(level));
        ...
        pending.Sort(static (a, b) =>
        {
            ...
            return a.Source.SourceIndex.CompareTo(b.Source.SourceIndex);
        });

        var output = new List<NativeCameraEvent>(pending.Count);
        AxisTarget targetX = new(true, 0.0f);
        AxisTarget targetY = new(true, 0.0f);
        ...
        foreach (PendingEvent pendingEvent in pending)
        {
            ...
            CameraState start = output.Count == 0
                ? new CameraState(pose.StationaryPlanet.X, pose.StationaryPlanet.Y, 0.0f, 1.0f)
                : Evaluate(output[^1], pendingEvent.StartTime, pose.StationaryPlanet.X, pose.StationaryPlanet.Y);

            if (item.Position.X is double x)
                targetX = ResolveAxis(item.RelativeTo, x, item.Floor, start.X, level, axisX: true);
            if (item.Position.Y is double y)
                targetY = ResolveAxis(item.RelativeTo, y, item.Floor, start.Y, level, axisX: false);
            if (item.Rotation is double rotation)
                targetRotation = (float)(rotation * Math.PI / 180.0);
            if (item.Zoom is double zoom)
                targetZoom = (float)Math.Clamp(zoom * 0.01, 0.01, 100.0);

            uint flags = 0u;
            if (targetX.PlayerRelative) flags |= NativeCameraEvent.FlagTargetPlayerX;
            if (targetY.PlayerRelative) flags |= NativeCameraEvent.FlagTargetPlayerY;

            output.Add(new NativeCameraEvent
            {
        ...
        bool axisX)
    {
        ...
        if (string.Equals(relativeTo, "LastPosition", StringComparison.OrdinalIgnoreCase))
            return new AxisTarget(false, start + component);
    ...
    private static CameraState Evaluate(NativeCameraEvent item, double chartTime, float playerX, float playerY)
    {
        float targetX = (item.Flags & NativeCameraEvent.FlagTargetPlayerX) != 0
            ? playerX + item.TargetX
            : item.TargetX;
        float targetY = (item.Flags & NativeCameraEvent.FlagTargetPlayerY) != 0
            ? playerY + item.TargetY
            : item.TargetY;
        double progress = item.DurationSeconds <= 1e-9
            ? 1.0
            : Math.Clamp((chartTime - item.StartTime) / item.DurationSeconds, 0.0, 1.0);
        float t = (float)ApplyEase(item.Ease, progress);
        return new CameraState(
            Lerp(item.StartX, targetX, t),
            Lerp(item.StartY, targetY, t),
            Lerp(item.StartRotation, item.TargetRotation, t),
            Lerp(item.StartZoom, item.TargetZoom, t));
    }

    private static List<CameraSourceEvent> BuildLiveEvents(LevelDocument level, CameraSourceData source)
    {
    ...
    {
        CameraSourcePosition position = TryGetPosition(obj["position"], out CameraSourcePosition parsedPosition)
            ? parsedPosition
            : source.Position;

        double? rotation = obj.ContainsKey("rotation") ? GetDouble(obj, "rotation") : source.Rotation;
        double? zoom = obj.ContainsKey("zoom") ? GetDouble(obj, "zoom") : source.Zoom;
        return source with
        {
            Floor = action.Floor,
            Active = action.Active,
    ...
    private static bool TryGetPosition(JsonNode? node, out CameraSourcePosition position)
    {
        ...
        position = new CameraSourcePosition(x, y);
        return true;
    }
    ...
    private static double ApplyEase(uint ease, double t)
    {
        ...
        const double c5 = 2.0 * Math.PI / 4.5;
        ...
        return ease switch
        {
            ...
            NativeCameraEvent.EaseInOutExpo => t <= 0.0 ? 0.0 : t >= 1.0 ? 1.0 : t < 0.5 ? Math.Pow(2.0, 20.0 * t - 10.0) / 2.0 : (2.0 - Math.Pow(2.0, -20.0 * t + 10.0)) / 2.0,
            NativeCameraEvent.EaseInCirc => 1.0 - Math.Sqrt(1.0 - t * t),
            NativeCameraEvent.EaseOutCirc => Math.Sqrt(1.0 - Math.Pow(t - 1.0, 2.0)),
            NativeCameraEvent.EaseInOutCirc => t < 0.5 ? (1.0 - Math.Sqrt(1.0 - Math.Pow(2.0 * t, 2.0))) / 2.0 : (Math.Sqrt(1.0 - Math.Pow(-2.0 * t + 2.0, 2.0)) + 1.0) / 2.0,
            ...
            NativeCameraEvent.EaseInOutBack => t < 0.5 ? Math.Pow(2.0 * t, 2.0) * ((c2 + 1.0) * 2.0 * t - c2) / 2.0 : (Math.Pow(2.0 * t - 2.0, 2.0) * ((c2 + 1.0) * (t * 2.0 - 2.0) + c2) + 2.0) / 2.0,
            NativeCameraEvent.EaseInElastic => t <= 0.0 ? 0.0 : t >= 1.0 ? 1.0 : -Math.Pow(2.0, 10.0 * t - 10.0) * Math.Sin((t * 10.0 - 10.75) * c4),
            NativeCameraEvent.EaseOutElastic => t <= 0.0 ? 0.0 : t >= 1.0 ? 1.0 : Math.Pow(2.0, -10.0 * t) * Math.Sin((t * 10.0 - 0.75) * c4) + 1.0,
            NativeCameraEvent.EaseInOutElastic => t <= 0.0 ? 0.0 : t >= 1.0 ? 1.0 : t < 0.5 ? -(Math.Pow(2.0, 20.0 * t - 10.0) * Math.Sin((20.0 * t - 11.125) * c5)) / 2.0 : Math.Pow(2.0, -20.0 * t + 10.0) * Math.Sin((20.0 * t - 11.125) * c5) / 2.0 + 1.0,
            NativeCameraEvent.EaseInBounce => 1.0 - OutBounce(1.0 - t),
            NativeCameraEvent.EaseOutBounce => OutBounce(t),
            NativeCameraEvent.EaseInOutBounce => t < 0.5 ? (1.0 - OutBounce(1.0 - 2.0 * t)) / 2.0 : (1.0 + OutBounce(2.0 * t - 1.0)) / 2.0,
            _ => t
        };
    }
